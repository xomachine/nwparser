...
proc readGeometry(fd: Stream, bonds: bool = true): Geometry =
  # no tag charge x y z
  ...
  if result.atoms.len > 1 and bonds:
...
proc findGeometry*(fd: Stream, bonds: bool = true): Geometry =
